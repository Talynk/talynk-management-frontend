...
const VideoLibraryPage = () => {
  const [allPosts, setAllPosts] = useState<ApproverPostType[]>([]);
  ...
  const fetchAllPosts = useCallback(async () => {
    ...
    try {
      ...
      const posts = await approverService.getApprovedPosts();
      ...
      <div className="px-9 py-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center p-8 text-gray-500 bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            />
            <p className="text-xl font-medium">No videos found in the library.</p>
            <p className="mt-2">The approval history is currently empty.</p>
          </div>
        ) : (
            ...
            {allPosts.map(post => (
              <div key={post.id} className="bg-white rounded-lg shadow p-4 border border-gray-200 transition-all duration-300 hover:shadow-md flex flex-col">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`text-xs font-medium px-2 py-1 rounded-full flex-shrink-0 ml-2 ${getStatusColor(post.status)}`}>
